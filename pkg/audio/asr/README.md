...
This package handles speech-to-text for NeoClaw voice input.
...
NeoClaw does not keep ASR API keys inside the `voice` section.
...
This is the recommended pattern because it is explicit, reusable, and consistent with the rest of NeoClaw's model configuration.
...
- You can omit `api_base` and NeoClaw will use Groq's default API base automatically.
...
NeoClaw currently supports three main ASR routes:
...
## How NeoClaw Chooses a Transcriber
...
2. If that resolved model is:
   - an `elevenlabs` provider model, NeoClaw uses the ElevenLabs transcriber.
   - an OpenAI-compatible Whisper model, NeoClaw uses the Whisper transcriber.
   - an audio-capable chat model, NeoClaw uses `AudioModelTranscriber`.
3. **Fallback path**: if `voice.model_name` is not set, NeoClaw performs a compatibility scan through `model_list` for legacy auto-detected ASR entries.
