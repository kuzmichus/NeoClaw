...
NeoClaw can run as a Delta Chat bot by launching a local
...
NeoClaw can find it automatically; otherwise set `rpc_server_path` to the
exact binary path.
...
The easiest setup is to let NeoClaw create a chatmail account in Delta
...
On startup, NeoClaw creates the account through `deltachat-rpc-server`, then
stops with an error that contains the generated address. Replace the relay
marker with that full email address and run NeoClaw again:
...
`password` is not needed for NeoClaw-created chatmail accounts. Omit it when
...
for classic email accounts that NeoClaw must configure itself. In that mode,
...
NeoClaw applies them on every startup, so changing the avatar path in config is
...
| `password` | No | Legacy only; required when NeoClaw must configure/reconfigure a classic mailbox itself |
...
With `email` set to `@server`, NeoClaw creates the chatmail account, prints
the generated full email in the startup error, and exits. Update `email` to that
full address and run NeoClaw again. On later runs, NeoClaw selects the
configured account by `email`, applies optional profile settings, marks it as a
bot, and starts IO.

With a new `data_dir` plus legacy `password`, NeoClaw can still configure a
...
it. On startup NeoClaw prints the bot invite link and QR code. Add the bot from
...
| `created chatmail account ...` | Replace the `@server` marker in `email` with the generated full email and run NeoClaw again |
