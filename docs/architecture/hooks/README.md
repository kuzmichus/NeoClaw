...
If you are developing NeoClaw itself rather than only validating the protocol, continue with the Go in-process example as well.
...
- Python process example
  Best for understanding the `JSON-RPC over stdio` protocol and verifying the message flow between NeoClaw and an external process
...
- NeoClaw starts the external process
...
The host does not currently accept new RPCs initiated by the process hook. In practice, that means an external hook can only respond to NeoClaw calls; it cannot call back into the host to send channel messages.
