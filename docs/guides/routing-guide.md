...
In NeoClaw, routing has two user-facing parts:
...
If no rule matches, NeoClaw falls back to the default agent.
...
- NeoClaw resolves the light model at startup; if it is invalid, routing is disabled
