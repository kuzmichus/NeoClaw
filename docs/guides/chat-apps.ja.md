...
NeoClaw は複数のチャットプラットフォームをサポートしており、Agent をどこにでも接続できます。
...
| **Pico**             | ⭐ 簡単            | NeoClaw ネイティブプロトコルチャネル     |                                                                                                               |
...
NeoClaw は統一されたコマンド定義を使用します。起動時に Telegram がサポートするコマンド（例: `/start`、`/help`、`/show`、`/list`、`/use`、`/btw`）を Bot コマンドメニューに自動登録し、メニュー表示と実際の動作を一致させます。
...
NeoClaw は 2 つの WhatsApp 接続方式をサポートしています：
...
NeoClaw は Tencent iLink 公式 API を使用して WeChat 個人アカウントへの接続をサポートしています。
...
NeoClaw は WebSocket/SDK モードで飛書に接続します — 公開 Webhook URL やコールバックサーバーは不要です。
...
NeoClaw は 3 種類の WeCom 統合をサポートしています：
...
OneBot は QQ ボット向けのオープンプロトコルです。NeoClaw は OneBot v11 互換の実装（例：[Lagrange](https://github.com/LagrangeDev/Lagrange.Core)、[NapCat](https://github.com/NapNeko/NapCatQQ)）に WebSocket で接続します。
...
任意の MQTT クライアントがブローカーを介して NeoClaw と通信できます。デバイスやサービスがブローカーにリクエストをパブリッシュし、NeoClaw がサブスクライブして処理し、レスポンスをパブリッシュして返します。
...
```
{prefix}/{agent_id}/{client_id}/request    # クライアント → NeoClaw
{prefix}/{agent_id}/{client_id}/response   # NeoClaw → クライアント
```
