...
NeoClaw sessions decide which messages share the same conversation history.
...
`session.dimensions` adds finer-grained isolation, but NeoClaw still keeps a baseline separation by:
...
`session.identity_links` helps when the same user may appear under multiple raw sender IDs and you want NeoClaw to treat them as one sender identity.
...
That is expected.
NeoClaw still separates sessions by channel even if you use `sender`.
...
NeoClaw keeps compatibility with older `agent:...` session keys while moving runtime storage to opaque canonical keys.
