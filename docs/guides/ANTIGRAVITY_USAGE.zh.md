...
# 在 NeoClaw 中使用 Antigravity 提供商

本指南介绍如何在 NeoClaw 中设置和使用 **Antigravity**（Google Cloud Code Assist）提供商。
...
5.  **从浏览器地址栏复制该最终 URL**。
6.  **将其粘贴回 NeoClaw 正在等待的终端中**。

NeoClaw 将自动提取授权码并完成流程。
...
*   **429 速率限制**：Antigravity 有严格的配额限制。如果触发限制，NeoClaw 将在错误消息中显示"重置时间"。
