...
NeoClaw stores data in your configured workspace (default: `~/.picoclaw/workspace`):
...
The `evolution` block controls NeoClaw's self-evolution runtime. When enabled, the agent records completed turns as learning records. In higher modes it can group repeated successful patterns, generate skill drafts, and optionally apply accepted drafts into workspace skills.
...
Use `observe` first if you want to inspect learning records without generating skill changes. Use `draft` when you want NeoClaw to prepare reviewable improvements. Use `apply` only when you are comfortable letting accepted drafts update workspace skills.

### Request Context Policy

`turn_profile` is an optional request context policy under `agents.defaults.turn_profile`. Leave it unset or set `"enabled": false` to keep NeoClaw's normal behavior. When `"enabled": true`, the same policy applies to every new turn.

Each block uses the same `mode` values:

| Mode | Meaning |
| --- | --- |
| `default` | Keep NeoClaw's normal behavior for that block. Missing blocks and missing `mode` fields are treated as `default`. |
...
| `system_prompt` | Whether NeoClaw injects its default identity, workspace instructions, memory, runtime context, and summary. External request system prompts are still allowed when this is `off`. |
...
When `system_prompt.mode` is `off`, tools are still visible, and no external system prompt is supplied, NeoClaw uses its existing tool-use rule as the minimal fallback prompt. If `tools.mode` is `off`, no fallback prompt is added.
...
rule matches, NeoClaw falls back to the configured default agent.
...
If `tools` is omitted from frontmatter, the agent gets the normal globally enabled tool set. If `tools` is present, NeoClaw registers only the listed runtime tools for that agent.
...
When an agent has spawnable peers and can call `spawn`, NeoClaw injects a structured agent registry into that agent's system prompt on every turn. No extra `list_agents` tool call is required.
...
NeoClaw runs in a sandboxed environment by default. The agent can only access files and execute commands within the configured workspace.
...
`read_file` has two mutually exclusive implementations selected by config. NeoClaw registers exactly one of them at startup:
...
The exec safety guard only inspects the command line NeoClaw launches directly. It does not recursively inspect child
...
* Run NeoClaw inside a container or VM if you need stronger isolation than the built-in guard provides.
...
NeoClaw can perform periodic tasks automatically. Create a `HEARTBEAT.md` file in your workspace:
...
| `volcengine` | LLM (Volcengine direct)                 | [volcengine.com](https://www.volcengine.com/activity/codingplan?utm_campaign=NeoClaw&utm_content=NeoClaw&utm_medium=devrel&utm_source=OWO&utm_term=NeoClaw) |
...
> **What's New?** NeoClaw now prefers explicit `provider` + native `model` configuration (for example `"provider": "zhipu", "model": "glm-4.7"`). The legacy single-field `provider/model` form remains supported for compatibility when `provider` is omitted.
...
NeoClaw supports separating sensitive data (API keys, tokens, secrets) from your main configuration by storing them in a `.security.yml` file.
...
| **VolcEngine (Doubao)** | `volcengine`      | `https://ark.cn-beijing.volces.com/api/v3`          | OpenAI    | [Get Key](https://www.volcengine.com/activity/codingplan?utm_campaign=NeoClaw&utm_content=NeoClaw&utm_medium=devrel&utm_source=OWO&utm_term=NeoClaw) |
...
- If `provider` is set, NeoClaw sends `model` unchanged.
- If `provider` is omitted, NeoClaw treats the first `/` segment in `model` as the provider and everything after that first `/` as the runtime model ID.
...
Provider streaming uses a double opt-in and is disabled by default. The agent only tries streaming when the current channel has `settings.streaming.enabled: true`, the active model entry has `streaming.enabled: true`, and both the provider and channel support streaming. If any condition is missing, NeoClaw uses the normal non-streaming request path.
...
Failure behavior is intentionally conservative: if streaming fails before any visible chunk is sent, NeoClaw retries once through the normal `Chat()` path. If a chunk has already been shown to the user, NeoClaw does not send a second non-streaming answer, because that would duplicate visible output.
...
With explicit `provider`, NeoClaw sends `openai/gpt-oss-20b` unchanged to LM Studio. The legacy compatibility form `"model": "lmstudio/openai/gpt-oss-20b"` still resolves to the same upstream model ID when `provider` is omitted.
...
With explicit `provider`, NeoClaw sends `model` unchanged. That means `"provider": "litellm", "model": "lite-gpt4"` sends `lite-gpt4`, while `"provider": "litellm", "model": "openai/gpt-4o"` sends `openai/gpt-4o`. The legacy compatibility forms `litellm/lite-gpt4` and `litellm/openai/gpt-4o` still resolve the same way when `provider` is omitted.

</details>

#### Load Balancing

Configure multiple endpoints for the same model name — NeoClaw will automatically round-robin between them:
...
NeoClaw routes providers by protocol family:
...
NeoClaw supports cron-style scheduled tasks via the `cron` tool. The agent can set, list, and cancel reminders or recurring jobs that trigger at specified times.
...
Command cron jobs can execute shell commands. By default, remote channels cannot schedule command jobs. To allow specific remote channels, set `command_allowed_remotes` to entries such as `"telegram"` or `"telegram:1234567890"`; use `"*"` only if every non-empty channel should be allowed. The `"*"` wildcard is potentially dangerous because any remote channel that can talk to NeoClaw can schedule shell commands. This does not bypass `allow_command`, `command_confirm`, or exec safety checks.
