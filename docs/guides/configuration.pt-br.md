...
O NeoClaw armazena dados no seu workspace configurado (padrão: `~/.picoclaw/workspace`):
...
`turn_profile` é uma política opcional em `agents.defaults.turn_profile` para controlar qual contexto cada novo turno carrega: histórico, prompt de sistema, prompts de skills e ferramentas permitidas. Sem essa configuração, ou com `"enabled": false`, o NeoClaw mantém o comportamento normal. Com `"enabled": true`, a política abaixo se aplica a cada novo turno.

Todos os blocos usam os mesmos valores de `mode`:

| Mode | Significado |
| --- | --- |
| `default` | Mantém o comportamento normal do NeoClaw. Blocos ausentes ou sem `mode` são tratados como `default`. |
...
| `system_prompt` | Injeção da identidade padrão do NeoClaw, instruções do workspace, memória, contexto de execução e resumo. Prompts de sistema externos ainda são permitidos quando este bloco está `off`. |
...
Quando `system_prompt.mode` é `off`, ferramentas continuam visíveis e nenhum prompt de sistema externo é fornecido, o NeoClaw reutiliza sua regra existente de uso de ferramentas como prompt mínimo de fallback. Se `tools.mode` é `off`, esse fallback não é adicionado.
...
O NeoClaw é executado em um ambiente sandbox por padrão. O agente só pode acessar arquivos e executar comandos dentro do workspace configurado.
...
O guard de segurança do exec inspeciona apenas a linha de comando que o NeoClaw executa diretamente. Ele não inspeciona recursivamente processos filhos gerados por ferramentas de desenvolvimento permitidas como `make`, `go run`, `cargo`, `npm run` ou scripts de build personalizados.
...
* Execute o NeoClaw dentro de um contêiner ou VM se precisar de isolamento mais forte do que o guard integrado oferece.
...
O NeoClaw pode executar tarefas periódicas automaticamente. Crie um arquivo `HEARTBEAT.md` no seu workspace:
...
| `volcengine` | LLM (Volcengine direto)                 | [volcengine.com](https://www.volcengine.com/activity/codingplan?utm_campaign=NeoClaw&utm_content=NeoClaw&utm_medium=devrel&utm_source=OWO&utm_term=NeoClaw) |
...
> **Novidade:** NeoClaw agora usa uma abordagem **centrada no modelo**. Basta especificar o formato `vendor/model` (ex.: `zhipu/glm-4.7`) para adicionar novos providers — **sem alterações de código!**
...
| **VolcEngine (Doubao)** | `volcengine/`   | `https://ark.cn-beijing.volces.com/api/v3`          | OpenAI    | [Obter](https://www.volcengine.com/activity/codingplan?utm_campaign=NeoClaw&utm_content=NeoClaw&utm_medium=devrel&utm_source=OWO&utm_term=NeoClaw) |
| **Antigravity**         | `antigravity/`  | Google Cloud                                        | Custom    | Somente OAuth                                                    |

#### Balanceamento de Carga

Configure múltiplos endpoints para o mesmo nome de modelo — NeoClaw fará round-robin automaticamente:
...
O comportamento de falha é intencionalmente conservador: se o streaming falhar antes de qualquer chunk visível ser enviado, o NeoClaw tenta novamente uma vez pelo caminho normal `Chat()`. Se um chunk já foi mostrado ao usuário, o NeoClaw não envia uma segunda resposta sem streaming, evitando output duplicado.

### Arquitetura de Providers

NeoClaw roteia providers por família de protocolo:
...
NeoClaw suporta tarefas agendadas via ferramenta `cron`.
