...
| `provider` | string | No | Preferred provider identifier. When present, NeoClaw sends `model` unchanged to that provider                                                                                                                                              |
...
By default, NeoClaw now forwards tool JSON Schemas unchanged.
...
- Default behavior is disabled. If you omit `tool_schema_transform`, NeoClaw sends the original tool schema.
- The setting is per model entry, so you can enable it only for the providers that need it.

#### Provider / Model Resolution

NeoClaw resolves `provider` and the runtime model ID using these rules:
...
If `voice.model_name` is not configured, NeoClaw will continue to fall back to Groq transcription when a Groq API key is available.
...
- NeoClaw still uses the OpenAI-compatible `/audio/speech` route for this setup.
...
With explicit `provider`, NeoClaw sends `openai/gpt-oss-20b` unchanged to the LM Studio server. The legacy compatibility form `"model": "lmstudio/openai/gpt-oss-20b"` still resolves to the same upstream model ID when `provider` is omitted.
...
NeoClaw already supports automatic failover when you configure `primary` + `fallbacks` in the agent model settings.
...
If you use key-level failover for the same model, NeoClaw can chain through additional key-backed candidates before moving to cross-model backups.
...
<div align="center">
  <img src="../../assets/logo.jpg" alt="NeoClaw Meme" width="512">
</div>
