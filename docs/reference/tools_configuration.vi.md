...
Cấu hình công cụ của NeoClaw nằm trong trường `tools` của `config.json`.
...
Theo mặc định, NeoClaw chặn các lệnh nguy hiểm sau:
...
Bộ bảo vệ exec chỉ xác thực lệnh cấp cao nhất được gửi đến NeoClaw. Nó **không** kiểm tra đệ quy các tiến trình con được tạo bởi các công cụ build hoặc script sau khi lệnh đó bắt đầu chạy.
