...
La configuration des outils de NeoClaw se trouve dans le champ `tools` de `config.json`.
...
Par défaut, NeoClaw bloque les commandes dangereuses suivantes :
...
Le garde exec ne valide que la commande de niveau supérieur envoyée à NeoClaw. Il n'inspecte **pas** récursivement les processus enfants générés par les outils de build ou les scripts après le démarrage de cette commande.
